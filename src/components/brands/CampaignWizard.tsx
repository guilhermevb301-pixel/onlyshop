...
import {
  MapPin, Users, Gift, Clock, ClipboardList, ArrowRight, ArrowLeft, Loader2, Sparkles, Rocket, Flag, Zap, UserCheck,
} from "lucide-react";
      ...
      <div className="min-h-[300px]">
        ...
        {wizardStep === 3 && (
          <div className="space-y-4 animate-fade-in">
            ...
            {/* Curadoria do perfil: por padrão VOCÊ aprova quem entra */}
            <div className="flex items-center justify-between gap-3 rounded-xl bg-white/[0.03] ring-1 ring-white/[0.06] px-3 py-2.5">
              <div className="min-w-0">
                <p className="text-xs font-semibold flex items-center gap-1.5"><UserCheck className="h-3.5 w-3.5 text-warning" /> Contratar todos automaticamente</p>
                <p className="text-[10px] text-muted-foreground/55 mt-0.5">Desligado (recomendado): quem se candidata fica aguardando o SEU OK antes de virar contratação. Ligado: entra direto.</p>
              </div>
              <Switch checked={form.auto_accept} onCheckedChange={(v) => update("auto_accept", v)} />
            </div>

            {/* Aprovação automática da ENTREGA (opt-in) */}
            <div className="flex items-center justify-between gap-3 rounded-xl bg-white/[0.03] ring-1 ring-white/[0.06] px-3 py-2.5">
              <div className="min-w-0">
                <p className="text-xs font-semibold flex items-center gap-1.5"><Zap className="h-3.5 w-3.5 text-accent" /> Aprovar entrega automaticamente</p>
